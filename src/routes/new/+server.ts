...
export const POST: RequestHandler = async (event) => {
	...
	const parseRes = z.object({
		...
		name, model, summary, prompt, attire
	})
	...
	if (!['@hf/mistralai/mistral-7b-instruct-v0.2', '@cf/meta/llama-2-7b-chat-int8', '@cf/tinyllama/tinyllama-1.1b-chat-v1.0'].includes(parseRes.data.model)) {
		return json({
