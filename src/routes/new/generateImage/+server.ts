...
export const POST: RequestHandler = async event => {
	const data = z
		...
		.max(512)
		.trim()
		.safeParse((await event.request.json()).attire)
	if (!data.success) {
		return json(
			{ error: data.error.issues.map(i => `Error: ${i.message}`).join('\n') },
			{ status: 400 }
		)
	}
	...
	try {
		const res = await fetch(
			`https://api.cloudflare.com/client/v4/accounts/${env.ACCOUNT_ID}/ai/run/@cf/bytedance/stable-diffusion-xl-lightning`,
			{
				method: 'POST',
				headers: {
